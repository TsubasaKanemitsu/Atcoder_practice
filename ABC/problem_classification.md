...
- パターン
    - 最適な最小化, 区間スケジューリング
- 考察
要望に答えるために必要な最小限の取り除くべき橋の本数を求める．
効率よく橋を取り除くためには，要望のあった区間で重なりのある橋を取り除いていくのが望ましい．
ということは，新しく取り除かなければならない時というのは，区間が被らないとき，すなわち1つ前の要望の島群(a - bのうちのbのこと)の終端が次の要望の島の始端(aのこと)より大きい場合である．
よって要望の島の終端は更新しつつ，前の要望の終端が次の要望の始端と重ならない時に橋を取り除くように回数をカウントすることで解答が求まる．
### 116C (復習)
- 解答時間
    - 解けなかった
- パターン
...
- パターン
    - 数え上げ
- 考察
各桁(2, 4, 6. 8. 10, 12)において前後の数字が同じになる数値の増加の間隔を実際に書き出して，与えられたNに対して間隔の値で割ることで与えられたNまでに存在する前後の数値が同じになる数の個数を数えることができる．
...
- パターン
    - DP
- 考察
この問題はK回移動した場合に，最終的にどの町に訪れているかを求める問題である．
制約としてKの最大値は10 ^ 18なので，愚直に移動した場合を求めることはできないことに気づかないといけない．ただし訪れる町の数であるNと移動回数のKの関係で，NよりKの方が小さい場合はNは2 * 10 ^ 5以下なので，愚直に計算した処理で最終到着地を求めることができる．
一方，N < Kの場合は，計算量を減らす工夫が必要となる．
そこで考えられるのは，訪れる町の周期性である．訪れる町の数より，移動回数が多いということは同じ町へ2度訪れるはずである．よってこの周期性を用いて，貪欲的に最終移動地点を求める．
最終地点を求めるためには，同じ町を訪れるループに入る最初の町の移動回数からK回移動するまでの回数までの間に複数回ループが入り，そのループの途中で最終地点に到達する．ということは，K回から同じ町を訪れるループに入る最初の町の移動回数の差分を1回あたりのループに必要な移動回数で割った余りの移動回数分，ループに入る最初の町から移動すれば，K回移動した場所と同じ場所へたどり着くと考えることができる．
...
- パターン
    - 数え上げ，数式の条件が重要
- 考察
abs(i - j) == Ai + Ajという条件から(i < j)の場合，i - Ai = j + Ajという条件に置き換えられる．これを1 ~ nまでの(i, Ai)の組みで計算していく．上記の条件に一致するのは必ず自分以外のペアとなるので，i - Aiとj + Ajの値が出た回数同士の組み合わせを考えることでペアの総数を求めることができる．
- データ構造
i - Aiとj + Ajがそれぞれ何回出現したかを取得するには，ハッシュテーブルを使用するのが望ましい．
- ペアの探索
i - Aiとj + Ajで求まる値はかならずN以下となるのでNまでの全探索でペアの総数を探索できる．
...
- パターン
    - 貪欲法
- 考察
[参考](https://atcoder.jp/contests/abc180/editorial/219)
